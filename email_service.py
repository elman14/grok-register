...
class CloudMailInbox:
    """CloudMail (Cloudflare Workers) 临时邮箱 — 通过公共 API token 创建地址 + 轮询收件箱。

    API:
      1. POST /api/public/addUser  → 创建邮箱地址 (public token auth)
      2. POST /api/public/emailList → 查询收件箱
    """

    def __init__(self, proxies: Any = None):
        self.base_url = str(os.getenv("CLOUDMAIL_BASE_URL") or "").strip().rstrip("/")
        self.public_token = str(os.getenv("CLOUDMAIL_PUBLIC_TOKEN") or "").strip()
        if not self.base_url:
            raise RuntimeError("缺少 CLOUDMAIL_BASE_URL")
        if not self.public_token:
            raise RuntimeError("缺少 CLOUDMAIL_PUBLIC_TOKEN")
        # CloudMail = CF Worker kita, langsung tanpa proxy
    ...
    def create_email(self) -> str:
        """创建随机邮箱地址,返回邮箱地址"""
        prefix = "g" + "".join(random.choices(_string.ascii_lowercase + _string.digits, k=10))
        # CloudMail domains: elf.biz.id, bangdodo.bond, sakithati.bond
        domains = ["elf.biz.id", "bangdodo.bond", "sakithati.bond"]
        domain = random.choice(domains)
        self.email = f"{prefix}@{domain}"
        self._password = "Gk" + "".join(random.choices(_string.ascii_letters + _string.digits, k=14)) + "!1"

        resp = self.session.post(
            f"{self.base_url}/api/public/addUser",
            json={"list": [{"email": self.email, "password": self._password}]},
            timeout=15,
        )
        if resp.status_code != 200:
            raise RuntimeError(f"CloudMail addUser 失败: {resp.status_code} {resp.text[:200]}")
        data = resp.json()
        if data.get("code") not in (200, 501):
            raise RuntimeError(f"CloudMail addUser 错误: {data}")
        return self.email

    def fetch_first_email(self) -> Optional[str]:
        """查询收件箱,返回第一封邮件内容 (或 None)"""
        if not self.email:
            return None
        try:
            resp = self.session.post(
            ...
            if not records:
                return None
            msg = records[0] if isinstance(records[0], dict) else None
            if not msg:
                return None
            subject = str(msg.get("subject") or "")
            content = str(msg.get("content") or "")
            text = str(msg.get("text") or "")
            # Strip HTML for OTP extraction
            import html as _html
            clean = re.sub(r"<style[^>]*>.*?</style>", "", content, flags=re.S | re.I)
            clean = re.sub(r"<script[^>]*>.*?</script>", "", clean, flags=re.S | re.I)
            clean = re.sub(r"<[^>]+>", " ", clean)
            clean = _html.unescape(clean)
            clean = re.sub(r"\s+", " ", clean).strip()
            return "\n".join([subject, text, clean]) if any([subject, text, clean]) else None
        except Exception as e:
...
class EmailService:
    ...
    def __init__(self, proxies: Any = None, provider: str = "luckmail"):
        ...
        if self.provider not in {"gptmail", "mailtm", "luckmail", "mailnest", "gmail", "tmail", "fce", "outlook", "cloudmail"}:
    ...
    def create_email(self):
        if self.provider == "mailtm":
            ...
            try:
                ...
                return None, None
        elif self.provider == "cloudmail":
            try:
                client = CloudMailInbox(self.proxies)
                email = client.create_email()
                token_like = {"provider": "cloudmail", "client": client, "email": email}
                print(f"[+] CloudMail 邮箱已创建: {email}")
                return token_like, email
            except Exception as e:
                print(f"[Error] 请求 CloudMail 出错: {e}")
                return None, None
        # gptmail: 使用 V2 API
        try:
    ...
    def fetch_first_email(self, token_like):
        try:
            ...
            if provider in ("mailtm", "luckmail", "gptmail-v2", "gmail", "tmail", "fce", "outlook", "cloudmail"):
